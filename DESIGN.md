...
**Topo de Matrículas** (desde 25/09/2026): sobre papel de caderno, título em duas batidas, lead e o link "Como funciona a matrícula ↓" (6 colunas), centralizados ao lado do card de agendamento (`AgendarWhatsApp`, 6 colunas). O "Prefere conversar antes?" saiu a pedido (o WhatsApp de conversa fica no botão flutuante e no rodapé). No celular: texto → card. O card é um contêiner de ação com friso e a sombra de flutuação (é o destaque do topo), cantos de 24 px: título em H3, a explicação em Small, a "tela" da conversa (cabeçalho com o nome da escola e "Mensagem que já vai escrita"; o balão `whatsapp-soft` à direita, sobre o papel de caderno) e o botão verde em largura total, com a microcopy centralizada.

**Trilha de passos** (`TrilhaPassos`, "Como funciona" de /matriculas, desde 25/09/2026): os mesmos passos em cards que ocupam a largura toda (4 colunas no desktop, 2 no tablet, empilhados no celular), em superfície com borda `line`, cantos de 14 px, padding de 24 → 32 px e altura mínima de 336 px no desktop. No topo de cada card, o ícone do passo num círculo de 64 px em `medio-soft` (WhatsApp, local, documento, check) e o numeral no tamanho `step`, em itálico azul; no pé, o título em H3 e o texto em Body `ink-muted`. O último passo ("Garanta a vaga") é o ponto de chegada: card em azul Anglo, círculo branco, numeral amarelo, texto branco. Uma linha tracejada em azul Anglo a 30% passa pelo centro dos círculos e aparece nos vãos entre os cards (horizontal no desktop, vertical no celular). A Home continua com a lista vertical (`Steps`), ao lado do card "Primeiro passo".
...
- **Eventos** (`CarrosselEventos`, seção em tinta antes do fecho; refeito em 25/09/2026): um evento por vez. Abaixo da cabeça em duas batidas, o palco (7 colunas) e o texto (5). No desktop, o palco sangra até a borda esquerda da tela, com cantos de 24 px só à direita, e tem no mínimo 34rem de altura; no celular, fica na margem, em 4:3, com cantos de 14 px. À direita: o ícone do evento num círculo de 56 px na cor da capa, a época do ano em Eyebrow amarelo (só quando confirmada), o nome em H2 Fraunces 600 branco, a descrição em Lead `on-ink-muted` (até 34ch) e o marcador do que falta (texto provisório, época, foto). Os textos de todos os eventos ocupam a mesma célula, então a coluna tem a altura do mais longo e não pula. Embaixo: as setas redondas (44 px, contorno branco a 35%, dão a volta nas pontas) com a posição "02 / 05" e, no desktop, o índice com os cinco nomes (linhas de 48 px separadas por branco a 15%, o atual em branco com um traço amarelo de 40 px à direita); no celular, cinco barras de 4 px (a atual em amarelo) com área de toque de 44 px. Deslizar o dedo no palco e as setas do teclado também trocam. Transição (só depois do primeiro clique): a capa nova varre por cima da anterior no sentido do friso (da esquerda ao avançar, da direita ao voltar, 600 ms), a foto assenta de 106% para 100% e o texto sobe 12 px em sequência de 60 ms; com movimento reduzido, troca sem animar. Um `aria-live` discreto anuncia o evento novo. Manual: o briefing veta o automático. Sem foto real, a capa é a cor do logo (azul, amarelo, azul-claro, vermelho, papel), o quadriculado e o ícone do evento em marca-d'água a 25%, centralizado na parte dentro da coluna; o nome não se repete na capa. Com foto, a imagem cobre o palco inteiro, sem escurecimento (não há texto sobre ela).
